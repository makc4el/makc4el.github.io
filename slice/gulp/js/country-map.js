...
var colors = ['#6659a3','#fff'];
      var randomColor = ['#34d6b6','#5f9cfd','#fb9d77','#669966','#FDCF8B','#FEF65B','#99668C'];
      var map;
      var counter = 0;
      var CountryCounter = 0;
      var MyCoontinents = localStorage.getItem('coontinents').split(',');
      function initialize() {   
        var myOptions = {
            scrollwheel: false,
      ...
      function drawMap(data) {
        ...
        var rows = data['rows'];
        for (var i in rows) {
          var Element = rows[i][0];
             var newCoordinatesChoose = [];
          for(j=0;j<countries.length;j++){
            if(rows[i][0] != countries[j].continent){
              var geo = rows[i][1]['geometries'];
              if(geo) {
              for (var j in geo) {
                    // newCoordinatesChoose.push(constructNewCoordinates(geo[j]));
                  }}
                  else {
                  // newCoordinatesChoose = constructNewCoordinates(rows[i][1]['geometry']);
                }
              }
            // var country = new google.maps.Polygon({
            //   paths: newCoordinatesChoose,
            //   strokeColor: '#000',
            //   strokeOpacity: 1,
            //   strokeWeight: 1,
            //   fillColor:'#fff',
