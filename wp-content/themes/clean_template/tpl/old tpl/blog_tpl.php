		...
		<div class="boats__inner">
			...
			<div class="posts-container">
				<div class="posts-container__item posts-container__item--two">
					<?php
					$myposts2 = get_posts(array('post_status' => 'publish', numberposts=>-1 ));
					$ch=0;
					$ids_kn='';
					foreach ($myposts2 as $k=>$post){
						$id_post = $post->ID;
				...
				<?php
				$myposts3 = get_posts(array('post_status' => 'publish', numberposts=>-1, 'exclude' => $ids_kn ));
				?>
				<div class="posts-container__item posts-container__item--three">
					<?php
					$ch2=0;
					foreach ($myposts3 as $k=>$post){
						$kn=0;
						$wpml_language_cod_post = wpml_get_language_information($post->ID)['language_code'];
						if($wpml_language_cod_post==ICL_LANGUAGE_CODE){
							if($ch2<=2){
					...
					<?php $kn++;} } wp_reset_postdata(); }?>
				</div>


				<?php  if($kn>0){ ?>
				<a href="<?php the_permalink(673) ?>" class="posts__view-all">
					<?php the_field('test'); ?>
				</a>
				<?php } ?>
			</div>
